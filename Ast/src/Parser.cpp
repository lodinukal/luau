...
ParseResult Parser::parse(const char* buffer, size_t bufferSize, AstNameTable& names, Allocator& allocator, ParseOptions options)
{
    ...
    const auto b = [&]()
    {
        ...
        result = ParseResult{root, lines, std::move(p.hotcomments), std::move(p.parseErrors), std::move(p.commentLocations), std::move(p.cstNodeMap)};
    };
    const auto c = [&](const std::exception& e)
    {
        if (const ParseError* perr = dynamic_cast<const ParseError*>(&e))
        {
            ...
            result = ParseResult{nullptr, 0, {}, p.parseErrors, {}, std::move(p.cstNodeMap)};
        }
        else
            LUAU_THROW(e);
    };
