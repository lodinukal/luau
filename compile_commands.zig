// modified from
// https://github.com/the-argus/zig-compile-commands/blob/main/compile_commands.zig
...
const CompileCommandStep = struct {
    step: std.Build.Step,
...
pub fn createStep(b: *std.Build, name: []const u8, targets: []const *std.Build.Step.Compile) *std.Build.Step {
    const step = b.allocator.create(CompileCommandStep) catch @panic("Allocation failure, probably OOM");
    step.compile_steps = b.allocator.dupe(*std.Build.Step.Compile, targets) catch @panic("Allocation failure, probably OOM");
    step.step = .init(.{
        .id = .custom,
        .name = "cc_file",
        .makeFn = makeCdb,
...
fn getCSources(b: *std.Build, steps: []const *std.Build.Step.Compile) []*AbsoluteCSourceFiles {
    ...
    for (compile_steps_list.items) |step| {
...
fn makeCdb(step: *std.Build.Step, _: std.Build.Step.MakeOptions) anyerror!void {
    const compile_command_step: *const CompileCommandStep = @fieldParentPtr("step", step);
    ...
    const c_sources = @call(.never_inline, getCSources, .{ step.owner, compile_command_step.compile_steps });
